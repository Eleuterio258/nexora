-- Repõe auth.criar_cargos_padrao(), reduzida a só 2 cargos escolares desde a
-- migração archive/20260629000093_cargos_turma — mesmo caso do schema
-- `hardware` e da tabela `rh.qr_tokens` restaurados nesta sessão, mas desta
-- vez o CREATE OR REPLACE FUNCTION da migração 093 substituiu por engano o
-- corpo inteiro da função definida em archive/20260629000081 (que tinha
-- Administrador, Gestor de TI, RH, Recrutamento, Comercial/CRM, Financeiro,
-- Contabilidade — em vez de só ACRESCENTAR Director de Turma/Chefe de Turma).
-- A versão mais completa e recente encontrada no histórico é
-- archive/20260712000121_auth_pin_admin_permission (593 linhas), usada aqui
-- como base — mas essa própria migração já tinha voltado a perder o cargo
-- "Chefe de Turma" e simplificado "Director de Turma", por isso essa parte
-- foi reconstruída a partir da versão mais completa de 093.
--
-- Consequência prática em produção: qualquer tenant criado depois de 093 (ou
-- cujo cargo "Administrador" tenha sido criado nessa janela) nunca teve
-- nenhuma permissão do módulo crm — nem sequer as genéricas antigas
-- (ver/criar/editar/apagar) que a rename de archive/20260629000046 traduzia
-- para ver_leads/gerir_leads/etc. É por isso que "CRM" nunca aparece na lista
-- de Módulos da app nexora_assiduidade, mesmo para um Administrador.
--
-- Diferença deliberada face ao histórico: em vez de seed genérico
-- (ver/criar/editar/apagar) + uma migração de rename separada (que nunca
-- cobriu ver_oportunidades/gerir_atividades/mover_leads), os cargos abaixo já
-- inserem directamente as acções finas que o router.go realmente verifica
-- (crm/handlers/{leads,oportunidades,atividades}.go): ver_leads, gerir_leads,
-- mover_leads, converter_leads, eliminar_leads, ver_oportunidades,
-- gerir_oportunidades, gerir_atividades.
...
BEGIN
    ...
    FROM (VALUES
        ...
        ('auditoria'), ('seguranca'), ('clientes'), ('vendas'),
    ...
    INSERT INTO auth.permissoes_cargo (cargo_id, modulo, acao)
    SELECT v_id, 'crm', a.acao FROM (VALUES
        ('ver_leads'), ('gerir_leads'), ('mover_leads'), ('converter_leads'), ('eliminar_leads'),
        ('ver_oportunidades'), ('gerir_oportunidades'), ('gerir_atividades')
    ) AS a(acao)
    ON CONFLICT DO NOTHING;
    ...
    INSERT INTO auth.permissoes_cargo (cargo_id, modulo, acao)
    SELECT v_id, m.modulo, a.acao
    FROM (VALUES ('clientes'), ('vendas')) AS m(modulo)
    CROSS JOIN (VALUES
        ('ver'),('criar'),('editar'),('apagar'),('exportar'),('aprovar'),('configurar'),('relatorios')
    ) AS a(acao)
    ON CONFLICT DO NOTHING;
    ...
    -- Gestor de Conta — "Carteira de clientes e oportunidades": acesso de
    -- trabalho completo ao funil (era ver/criar/editar/apagar genérico).
    INSERT INTO auth.cargos (tenant_id, nome, descricao)
    ...
    INSERT INTO auth.permissoes_cargo (cargo_id, modulo, acao) VALUES
        ...
        (v_id, 'crm', 'ver_leads'),         (v_id, 'crm', 'gerir_leads'),
        (v_id, 'crm', 'mover_leads'),       (v_id, 'crm', 'converter_leads'),
        (v_id, 'crm', 'eliminar_leads'),    (v_id, 'crm', 'ver_oportunidades'),
        (v_id, 'crm', 'gerir_oportunidades'),(v_id, 'crm', 'gerir_atividades')
    ON CONFLICT DO NOTHING;

    -- ───────────────────────────────────────────────────────────────────────

    -- Técnico Comercial — "Orçamentos e seguimento de oportunidades": só
    -- consulta e acompanha (era ver/editar genérico, sem apagar).
    INSERT INTO auth.cargos (tenant_id, nome, descricao)
    ...
    INSERT INTO auth.permissoes_cargo (cargo_id, modulo, acao) VALUES
        ...
        (v_id, 'crm', 'ver_leads'),          (v_id, 'crm', 'gerir_leads'),
        (v_id, 'crm', 'ver_oportunidades'),  (v_id, 'crm', 'gerir_oportunidades'),
    ...
    -- Director de Turma — versão completa (arquivada em 093; a de 000121
    -- tinha perdido "relatorios"/"gerir_comunicacao").
    INSERT INTO auth.cargos (tenant_id, nome, descricao)
    ...
    -- Chefe de Turma — presente em 093, tinha desaparecido em 000121.
    INSERT INTO auth.cargos (tenant_id, nome, descricao)
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Reparação de dados (idempotente): tenants cujo cargo "Administrador" já
-- existia mas foi criado enquanto a função esteve reduzida (093→agora) nunca
-- receberam nenhuma permissão crm — nem sequer as genéricas antigas, que só
-- existiam se o cargo tivesse sido criado antes de 093. Sem a coluna
-- created_at de auth.permissoes_cargo não há forma de distinguir automagicamente
-- "nunca teve" de "teve e foi removida de propósito", por isso este backfill
-- só actua quando o cargo Administrador não tem NENHUMA linha modulo='crm' —
-- o caso demonstrável de regressão, sem tocar em cargos já configurados.
-- ─────────────────────────────────────────────────────────────────────────────
INSERT INTO auth.permissoes_cargo (cargo_id, modulo, acao)
SELECT c.id, 'crm', a.acao
  FROM auth.cargos c
  CROSS JOIN (VALUES
      ('ver_leads'), ('gerir_leads'), ('mover_leads'), ('converter_leads'), ('eliminar_leads'),
      ('ver_oportunidades'), ('gerir_oportunidades'), ('gerir_atividades')
 ...
 WHERE c.nome = 'Administrador'
   AND NOT EXISTS (
       SELECT 1 FROM auth.permissoes_cargo pc
        WHERE pc.cargo_id = c.id AND pc.modulo = 'crm'
   )
ON CONFLICT DO NOTHING;

-- ─────────────────────────────────────────────────────────────────────────────
-- Reparação de dados: cargos com linhas crm ainda no esquema genérico antigo
-- (ver/criar/editar/apagar/gerir) — traduz para as acções finas, cobrindo
-- também ver_oportunidades/gerir_atividades/mover_leads, que a rename
-- arquivada (20260629000046) nunca tinha coberto.
-- ─────────────────────────────────────────────────────────────────────────────
INSERT INTO auth.permissoes_cargo (cargo_id, modulo, acao)
SELECT DISTINCT pc.cargo_id, 'crm', novo.acao
  FROM auth.permissoes_cargo pc
  CROSS JOIN LATERAL (
      SELECT CASE pc.acao
          WHEN 'ver'    THEN 'ver_leads'
          WHEN 'criar'  THEN 'gerir_leads'
          WHEN 'editar' THEN 'gerir_oportunidades'
          WHEN 'apagar' THEN 'eliminar_leads'
          WHEN 'gerir'  THEN 'converter_leads'
      END AS acao
  ) AS novo
 WHERE pc.modulo = 'crm'
   AND pc.acao IN ('ver','criar','editar','apagar','gerir')
   AND novo.acao IS NOT NULL
ON CONFLICT DO NOTHING;

DELETE FROM auth.permissoes_cargo
 WHERE modulo = 'crm' AND acao IN ('ver','criar','editar','apagar','gerir');

-- ─────────────────────────────────────────────────────────────────────────────
-- Aplicar aos tenants que ainda não têm cargo "Administrador" (novos desde a
-- regressão, ou nunca provisionados) — não mexe em tenants já configurados.
-- ─────────────────────────────────────────────────────────────────────────────
