...
final class GestaoEscolarController
{
    public function escolarOperacao(Request $request, AdminApiDependencies $d): ApiResult
    {
        ...
        $action = str_ends_with($operation, '.create')
            ? 'criar'
            : ((str_ends_with($operation, '.view') || str_ends_with($operation, '.receipt')) ? 'ver' : 'editar');
        $authorization = new PhpSessionAuthorization();
        if (!$authorization->can('gestao-escolar', $action)) {
