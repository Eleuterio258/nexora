...
/**
 * Dashboard POS — Portal Gerente
 * Replica funcionalidade de pages/pos_dashboard.php usando as novas rotas /pos/*.
 */

$hoje = date('Y-m-d');

$vendas = $app->pos->listVendas(['data_inicio' => $hoje, 'data_fim' => $hoje]);

$sessaoResp = $app->nexora->call('GET', '/api/pos/sessoes/atual');
$sessaoAtual = ($sessaoResp['status'] === 200) ? $sessaoResp['body'] : null;

$terminais = $app->pos->listTerminais();
...
$activePage = 'pos_manager_dashboard';
$breadcrumb = [['Admin', '/nexora/'], ['POS', '/pos/gerente/dashboard'], ['Dashboard', '']];
...
<div class="adm-page-header">
    ...
    <div class="adm-page-header-actions">
        ...
        <a href="<?= htmlspecialchars($app->routes->path('pos_operator_terminal')) ?>" class="adm-btn adm-btn-primary">
...
<div class="adm-stats-grid" style="margin-bottom:var(--adm-sp-6)">
    ...
    <div class="adm-stat-card">
        <div class="adm-stat-icon adm-stat-icon--yellow"><i class="fa-solid fa-chart-line" style="font-size:1.1rem"></i></div>
        <div class="adm-stat-info">
...
<div style="display:grid;grid-template-columns:300px 1fr;gap:var(--adm-sp-6);margin-bottom:var(--adm-sp-6)">
    <div class="adm-card">
        <div class="adm-card-header"><h2 class="adm-card-title">Formas de Pagamento</h2></div>
        <div class="adm-card-body">
            ...
            <?php foreach ($metodosCount as $metodo => $valor):
                $pct = $totalMetodos > 0 ? round($valor / $totalMetodos * 100) : 0; ?>
    ...
    <div class="adm-card">
        <div class="adm-card-header">
            ...
            <div class="adm-card-actions">
                <a href="<?= htmlspecialchars($app->routes->path('pos_manager_sales')) ?>" class="adm-btn adm-btn-ghost adm-btn-sm">Ver todas</a>
            </div>
        </div>
        <?php if (empty($vendas)): ?>
        ...
        <div class="adm-table-wrap">
            <table class="adm-table">
                <thead>
                    <tr><th>Ref.</th><th>Hora</th><th>Método</th><th>Items</th><th>Total</th><th>Estado</th></tr>
                </thead>
                <tbody>
...
<div class="adm-card">
    <div class="adm-card-header">
        ...
        <div class="adm-card-actions">
            <a href="<?= htmlspecialchars($app->routes->path('pos_admin_terminals')) ?>" class="adm-btn adm-btn-ghost adm-btn-sm">Gerir terminais</a>
        </div>
    </div>
    ...
    <div class="adm-table-wrap">
        <table class="adm-table">
            ...
            <tbody>
            <?php foreach ($terminais as $t): $ativo = !empty($t['activo']); ?>
            <tr>
                <td class="adm-fw-600"><?= htmlspecialchars($t['nome'] ?? '—') ?></td>
                <td class="adm-text-muted"><?= htmlspecialchars($t['localizacao'] ?? '—') ?></td>
                <td><span class="adm-badge <?= $ativo ? 'adm-badge--green' : 'adm-badge--gray' ?>"><?= $ativo ? 'Activo' : 'Inactivo' ?></span></td>
                <td>
                    <a href="<?= htmlspecialchars($app->routes->path('pos_operator_terminal')) ?>" class="adm-btn adm-btn-ghost adm-btn-sm">
