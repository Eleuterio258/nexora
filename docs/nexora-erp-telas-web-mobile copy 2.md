...
Web:
- Layout desktop
- Menus, tabelas, filtros, botões e painéis
- Comportamento esperado

Mobile:
- Layout mobile
- Navegação inferior ou menu lateral
- Cards, listas, formulários simplificados
- Ações principais
...
Repetir este formato para todas as telas.

Requisitos de design:
- Interface limpa, profissional e empresarial
- Não usar estilo de landing page
- Priorizar produtividade, leitura rápida e uso diário
- Web deve usar sidebar, topbar, tabelas, filtros e dashboards
- Mobile deve usar navegação inferior, listas em cards, ações rápidas e formulários em etapas quando necessário
- Usar ícones em botões de ação
- Evitar excesso de cores
- Usar estados claros para documentos, pagamentos, stock e permissões
- Suportar modo claro e modo escuro
- Pensar em utilizadores administrativos, vendedores, caixas, gestores financeiros e contabilidade

Comece pelos módulos core de plataforma:
1. Empresas
2. Autenticação
3. Autorização
4. Utilizadores
...
Entregue a resposta em português, organizada por módulo e por tela.
```

Também pode usar esta versão mais direta:

```text
Crie todas as telas web e mobile do Nexora ERP, módulo por módulo, começando pelo core.
...
Módulos:
empresas, autenticacao, autorizacao, utilizadores, auditoria, sistema-configuracao, gestao-clientes, gestao-produtos, gestao-stock, modulo-faturacao, tesouraria, financeiro e contabilidade.

A interface deve ser profissional, responsiva, moderna, simples de usar e adequada para ERP empresarial.
```

Minha recomendação: começar com o primeiro prompt, porque ele força a IA a gerar as telas com mais detalhe e separação entre **web** e **mobile**.
**Módulo: Autenticação**
...
- Limpeza por retenção deve ser controlada e auditada.**Próximo Módulo: Sistema e Configuração**
...
- Dados sensíveis devem ser protegidos.**Próximo Módulo: Gestão de Clientes**
...
- Regras com sobreposição devem ter prioridade clara.**Próximo Módulo: Gestão de Clientes**
...
- Desconto não pode exceder limite máximo configurado.
- Descontos ativos devem ser considerados na faturação.
- Regras com sobreposição devem ter prioridade clara.**Próximo Módulo: Gestão de Produtos**
...
- Vendas vêm de faturação/POS.**Próximo Módulo: Faturação**
...
- Valores devem considerar notas de crédito e anulações.**Próximo Módulo: Financeiro**
...
Regras:
- Exportações devem ser auditadas.
- Relatórios devem respeitar permissões por módulo e tenant.**Próximo Módulo: Contabilidade**
...
- Exportações devem ser auditadas.
- Documentos oficiais devem usar dados fechados quando aplicável.**Próximo Módulo: Impostos**
...
Observação: `seguranca` está marcado como **descontinuado**, porque foi substituído por `autenticacao`, `autorizacao` e `auditoria`. Então, se removermos `seguranca`, faltam **9 módulos úteis**.**Próximo Módulo: Multi-Moeda**
...
- Exportações devem ser auditadas.**Próximo Módulo: Compras**
...
- Exportações devem ser auditadas.**Próximo Módulo: POS**
...
- Exportações devem ser auditadas.**Próximo Módulo: Logística**
...
- Exportações devem ser auditadas.**Próximo Módulo: CRM**
...
- Exportações devem ser auditadas.**Próximo Módulo: Recursos Humanos**
...
- Exportações devem ser auditadas.**Próximo Módulo: Assinaturas**
...
- Exportações devem ser auditadas.Faltam **3 módulos úteis** para completar a lista:

1. `gestao-escolar`
2. `centros-custo`
3. `seguranca`

Mas `seguranca` está **descontinuado**, porque foi substituído por:
...
Então, na prática, faltam só **2 módulos importantes**:
...
recursos-humanos
assinaturas
```**Próximo Módulo: Centros de Custo**
...
- Exportações devem ser auditadas.Sim. O módulo `seguranca` foi **descontinuado**.

Ele foi substituído por três módulos separados e mais corretos:
...
Então **não precisa criar telas novas para `seguranca`**.

Status final:

```text
Módulos úteis concluídos: todos
Módulo descontinuado: seguranca
Faltam módulos: 0
```

Agora o próximo passo ideal é organizar tudo em um documento final com:

```text
1. Mapa completo de módulos
2. Lista final de telas Web
3. Lista final de telas Mobile
4. Permissões por módulo
5. Ordem recomendada de implementação
6. MVP inicial do Nexora ERP
```