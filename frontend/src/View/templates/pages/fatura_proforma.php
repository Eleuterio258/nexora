...
$cliente = $app->nexora->call('GET', '/api/clientes/' . $fatura['customer_id'])['body'] ?? [];
...
<div class="head">
    ...
    <div style="text-align:right">
        <h1><?= htmlspecialchars($fatura['numero']) ?></h1>
        <div class="muted">Data: <?= date('d/m/Y', strtotime($fatura['invoice_date'])) ?></div>
        <?php if (!empty($fatura['due_date'])): ?><div class="muted">Vencimento: <?= date('d/m/Y', strtotime($fatura['due_date'])) ?></div><?php endif; ?>
    </div>
</div>

<div><strong>Cliente:</strong> <?= htmlspecialchars($cliente['nome'] ?? ('#' . $fatura['customer_id'])) ?></div>
...
<table>
    ...
    <tr class="total-row">
        ...
        <td class="right"><?= number_format((float) $fatura['total'], 2, ',', '.') ?> <?= htmlspecialchars($fatura['moeda']) ?></td>
    </tr>
    </tbody>
</table>

<?php if (!empty($fatura['observacoes'])): ?>
<p class="muted"><?= nl2br(htmlspecialchars($fatura['observacoes'])) ?></p>
<?php endif; ?>
