-- Reconciliação: a tabela pos_payments foi criada por engano numa
-- iteração anterior desta migration; o esquema real usa
-- pos_sale_payments (ver abaixo). Está vazia, remoção segura.
DROP TABLE IF EXISTS pos.pos_payments CASCADE;

CREATE SCHEMA IF NOT EXISTS pos;

SET search_path TO pos, faturacao, produtos, stock, financeiro, public;
...
-- ============================================================
-- CATÁLOGO POS (preço e código de barras por produto/variante)
-- ============================================================
...
-- ============================================================
-- NUMERAÇÃO: novo tipo 'VD' (Venda POS) em invoice_series
-- ============================================================

ALTER TABLE invoice_series DROP CONSTRAINT IF EXISTS invoice_series_tipo_check;
ALTER TABLE invoice_series ADD CONSTRAINT invoice_series_tipo_check
    CHECK (tipo IN ('ORC', 'ENC', 'GR', 'FT', 'NC', 'RB', 'VD'));
...
CREATE INDEX IF NOT EXISTS idx_pos_sales_session ON pos_sales (pos_session_id);
