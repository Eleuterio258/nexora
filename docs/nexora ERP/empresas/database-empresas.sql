-- Modulo de Empresas e Multi-Tenant para PostgreSQL
...
CREATE TABLE IF NOT EXISTS companies (
    ...
    tipo VARCHAR(30) NOT NULL DEFAULT 'empresa' CHECK (tipo IN ('empresa', 'organizacao', 'holding', 'filial_independente')),
    status VARCHAR(20) NOT NULL DEFAULT 'ativa' CHECK (status IN ('ativa', 'suspensa', 'inativa')),
...
CREATE TABLE IF NOT EXISTS company_addresses (
    ...
    company_id BIGINT NOT NULL,
    branch_id BIGINT,
    tipo VARCHAR(30) NOT NULL DEFAULT 'principal' CHECK (tipo IN ('principal', 'fiscal', 'entrega', 'filial', 'cobranca')),
...
CREATE TABLE IF NOT EXISTS company_contacts (
    ...
    company_id BIGINT NOT NULL,
    branch_id BIGINT,
    tipo VARCHAR(30) NOT NULL DEFAULT 'geral' CHECK (tipo IN ('geral', 'financeiro', 'comercial', 'suporte', 'rh')),
    nome VARCHAR(150),
    telefone VARCHAR(30),
    email VARCHAR(150),
    principal BOOLEAN NOT NULL DEFAULT FALSE,
...
CREATE TABLE IF NOT EXISTS company_documents (
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    company_id BIGINT NOT NULL,
    tipo VARCHAR(30) NOT NULL CHECK (tipo IN ('alvara', 'certidao', 'contrato_social', 'licenca', 'outro')),
    numero VARCHAR(100),
    ficheiro_url TEXT,
    emitido_em DATE,
    expira_em DATE,
...
CREATE TABLE IF NOT EXISTS company_licenses (
    ...
    expira_em DATE,
    status VARCHAR(20) NOT NULL DEFAULT 'ativa' CHECK (status IN ('ativa', 'expirada', 'suspensa')),
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
