...
func (h *Handler) ListarNotificacoes(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.db.Query(r.Context(),
		"SELECT id, tipo, titulo, mensagem, lida, lida_em, created_at FROM user_notifications WHERE "+
...
func (h *Handler) CriarNotificacao(w http.ResponseWriter, r *http.Request) {
	...
	h.db.QueryRow(r.Context(), `
		INSERT INTO user_notifications (user_id, tipo, titulo, mensagem)
		VALUES ($1, $2, $3, $4) RETURNING id`,
		userID, body.Tipo, body.Titulo, body.Mensagem).Scan(&id)
...
func (h *Handler) MarcarNotificacaoLida(w http.ResponseWriter, r *http.Request) {
	...
	tag, _ := h.db.Exec(r.Context(), `
		UPDATE user_notifications SET lida = TRUE, lida_em = NOW()
