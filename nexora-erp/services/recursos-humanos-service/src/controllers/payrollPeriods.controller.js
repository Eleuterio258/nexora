...
async function listar(req, res, next) {
  try {
    const { rows } = await db.query(
      `SELECT * FROM payroll_periods WHERE tenant_id = $1 ORDER BY ano DESC, mes DESC`,
      [req.user.tenantId]
    );
    res.json(rows);
  } catch (err) {
...
async function criar(req, res, next) {
  try {
    ...
    if (!ano || !mes || !data_inicio || !data_fim) {
      return res.status(400).json({ error: 'ano, mes, data_inicio e data_fim sao obrigatorios' });
    }

    const { rows } = await db.query(
      `INSERT INTO payroll_periods (tenant_id, ano, mes, data_inicio, data_fim)
       VALUES ($1,$2,$3,$4,$5)
       RETURNING *`,
      [req.user.tenantId, ano, mes, data_inicio, data_fim]
    );
