...
class GradeEntryClassScreen extends StatelessWidget {
  ...
  static const _turmas = [
    _Turma(nome: '10ª Classe A', disciplina: 'Matemática', pendentes: 3, total: 28),
    _Turma(nome: '10ª Classe B', disciplina: 'Matemática', pendentes: 0, total: 26),
    ...
    _Turma(nome: '12ª Classe A', disciplina: 'Matemática', pendentes: 0, total: 20),
              ...
              children: [
                Padding(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: const [
                      Text('Notas', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: _navy)),
                      SizedBox(height: 2),
                      Text('Seleccione uma turma para lançar notas', style: TextStyle(fontSize: 13, color: _grey)),
                    ],
                  ),
                ),
                ...
                Expanded(
                  child: ListView.separated(
                    ...
                    itemBuilder: (context, i) => _GradeTurmaCard(
                      turma: _turmas[i],
                      onTap: () => Navigator.pushNamed(context, AppRoutes.gradeEntryStudents),
                    ),
                  ),
                ),
              ],
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(turma.disciplina, style: const TextStyle(fontSize: 13, color: _grey)),
                  const SizedBox(height: 6),
                  ClipRRect(
                    borderRadius: BorderRadius.circular(4),
                    child: LinearProgressIndicator(
                      value: hasPendentes ? (turma.total - turma.pendentes) / turma.total : 1.0,
                      backgroundColor: const Color(0xFFEEEEF0),
                      valueColor: AlwaysStoppedAnimation<Color>(hasPendentes ? const Color(0xFFF59E0B) : _green),
                      minHeight: 4,
                    ),
                  ),
                  const SizedBox(height: 4),
                  Text(
                    hasPendentes ? '${turma.pendentes} notas por lançar' : 'Todas as notas lançadas',
                    style: TextStyle(
                      fontSize: 11,
...
class _Turma {
  const _Turma({required this.nome, required this.disciplina, required this.pendentes, required this.total});
