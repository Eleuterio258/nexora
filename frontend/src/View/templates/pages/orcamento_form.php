<?php
    ...
    $estadoBadge = $estadoBadges[$orcamento['status'] ?? 'rascunho'] ?? ['adm-badge--gray', $orcamento['status'] ?? ''];

    $csrf       = $app->security->csrfToken();
    $pageTitle  = $isEdit ? 'Orçamento ' . $orcamento['numero'] : 'Novo Orçamento';
    $activePage = 'orcamentos';
    $breadcrumb = [['Admin', '/nexora/'], ['Faturação', ''], ['Orçamentos', '/nexora/faturacao/orcamentos'], [$isEdit ? $orcamento['numero'] : 'Novo', '']];
...
<div class="adm-page-header" style="align-items:flex-start">
    <div style="display:flex;align-items:center;gap:var(--adm-sp-3)">
        <h1 class="adm-page-title" style="margin:0"><?php echo $isEdit ? htmlspecialchars($orcamento['numero']) : 'Novo Orçamento' ?></h1>
        <?php if ($isEdit): ?>
    ...
    <div class="adm-page-header-actions">
        <?php if ($isEdit): ?>
        <a href="/nexora/faturacao/orcamentos/proforma?id=<?php echo htmlspecialchars($idHash) ?>" target="_blank" class="adm-btn adm-btn-outline adm-btn-sm">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg>
            Imprimir
        </a>
        <?php endif; ?>
        <a href="/nexora/faturacao/orcamentos" class="adm-btn adm-btn-outline adm-btn-sm">
...
<?php if (! $isEdit): ?>
<div class="adm-card">
    <div class="adm-card-header"><h2 class="adm-card-title">Novo Orçamento</h2></div>
    <div class="adm-card-body">
        ...
        <button class="adm-btn adm-btn-primary" id="btnSave" onclick="saveOrcamento()">
    ...
    <div class="adm-card-body">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--adm-sp-5)">
            ...
            <div class="adm-detail-pair">
                ...
                <span class="adm-detail-pair-value"><?php echo $orcamento['created_at'] ? date('d/m/Y', strtotime($orcamento['created_at'])) : '—' ?></span>
            </div>
            <div class="adm-detail-pair">
                <span class="adm-detail-pair-label">Validade</span>
                <span class="adm-detail-pair-value"><?php echo $orcamento['validade'] ? date('d/m/Y', strtotime($orcamento['validade'])) : '—' ?></span>
            </div>
            <div class="adm-detail-pair">
                <span class="adm-detail-pair-label">Moeda</span>
                <span class="adm-detail-pair-value"><?php echo htmlspecialchars($orcamento['moeda']) ?></span>
            </div>
            <div class="adm-detail-pair">
                <span class="adm-detail-pair-label">Total</span>
                <span class="adm-detail-pair-value adm-fw-600"><?php echo number_format((float) $orcamento['total'], 2, ',', '.') ?> <?php echo htmlspecialchars($orcamento['moeda']) ?></span>
            </div>
        </div>
        ...
        <div class="adm-detail-pair" style="margin-top:var(--adm-sp-4)">
            ...
            <span class="adm-detail-pair-value"><?php echo nl2br(htmlspecialchars($orcamento['observacoes'])) ?></span>
        </div>
        <?php endif; ?>
    </div>
</div>
...
<aside>
    <div class="adm-card">
        <div class="adm-card-header"><h2 class="adm-card-title">Estado do Orçamento</h2></div>
        <div class="adm-card-body">
            ...
            <div style="display:flex;flex-direction:column;gap:var(--adm-sp-2)">
                <?php if (($orcamento['status'] ?? '') === 'rascunho'): ?>
                <button class="adm-btn adm-btn-outline adm-btn-sm" onclick="changeEstado('enviar')" style="justify-content:flex-start">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
                    Enviar Orçamento
                </button>
                <?php elseif (($orcamento['status'] ?? '') === 'enviado'): ?>
                <button class="adm-btn adm-btn-outline adm-btn-sm" onclick="changeEstado('aprovar')" style="justify-content:flex-start;color:var(--adm-green-dark)">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg>
                    Aprovar Orçamento
                </button>
                <button class="adm-btn adm-btn-outline adm-btn-sm" onclick="changeEstado('rejeitar')" style="justify-content:flex-start;color:var(--adm-red)">
...
<script>
const ORCAMENTO_ID = <?php echo $isEdit ? $id : 'null' ?>;
...
async function saveOrcamento() {
    ...
    try {
        ...
        if (data.ok) {
            window.location.href = '/nexora/faturacao/orcamentos/form?id=' + nexoraEncodeId(data.id);
        } else {
...
function deleteItem(itemId) {
    openConfirm(
        'Eliminar item',
        'Eliminar este item do orçamento?',
        async () => {
            try {
                const res  = await fetch('/nexora/api/orcamento_item_delete', {
                    method: 'POST', headers: {'Content-Type':'application/json'},
                    body: JSON.stringify({quote_id: ORCAMENTO_ID, item_id: itemId, csrf: CSRF})
                });
                const data = await res.json();
                if (data.ok) {
...
// ── Estado ───────────────────────────────────────────────────
const ESTADO_VERBOS = { enviar: 'enviar', aprovar: 'aprovar', rejeitar: 'rejeitar' };

function changeEstado(action) {
    openConfirm(
        'Alterar estado',
        'Pretende ' + ESTADO_VERBOS[action] + ' este orçamento?',
        async () => {
            try {
