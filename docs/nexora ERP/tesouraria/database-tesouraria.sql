-- Modulo de Tesouraria para PostgreSQL
...
CREATE TABLE IF NOT EXISTS movimentos_financeiros (
    ...
    CONSTRAINT fk_movimentos_financeiros_conta FOREIGN KEY (conta_bancaria_id) REFERENCES contas_bancarias(id) ON DELETE SET NULL,
    CONSTRAINT fk_movimentos_financeiros_caixa FOREIGN KEY (caixa_id) REFERENCES caixas(id) ON DELETE SET NULL
);
...
CREATE INDEX IF NOT EXISTS idx_movimentos_financeiros_tenant_id ON movimentos_financeiros (tenant_id);
