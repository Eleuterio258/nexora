...
- Verificado: `go build ./...` limpo, `compileDebugKotlin` sucesso.
- **Efeito colateral corrigido**: `/biometric/verify` tinha `@limiter.limit("30/minute")` por IP (`app/limiter.py`, `get_remote_address`) — fazia sentido quando cada telemóvel chamava directamente (limite por dispositivo). Com o ERP como único chamador, todos os tenants passam a partilhar o mesmo IP de origem, e 30/min para a plataforma inteira rejeitaria verificações legítimas em hora de ponta. Subido para `1000/minute` (`app/routers/biometric.py`).

## 13. Portal PHP (`frontend/`) — migração OAuth2 ainda não iniciada (levantamento 2026-07-28)

A secção 11 diz "quem migra para `/oauth/token` é a app e o portal PHP" — a app já migrou (secção 8.4/11); o portal PHP (`frontend/`, `E258Tech\Infrastructure\Nexora\NexoraClient`) **ainda não**. Levantamento do bloqueio real, sem alterar código:

- `NexoraClient::authenticate()`/`refresh()` continuam a chamar `POST /api/auth/login` / `POST /api/auth/refresh` (JWT HS256 antigo) — nenhuma chamada a `/oauth/token` existe ainda no PHP.
- O Go já tem um cliente `web-erp` seedado (`auth.oauth_clients`, `client_type='confidential'`, grants `password`/`refresh_token`/`authorization_code`, `is_first_party=TRUE` — migration `20260727000001_oauth2_authorization_server.up.sql:100`), mas **`client_secret_hash` está `NULL`** — nenhum segredo foi gerado nem definido. Enquanto estiver `NULL`, `authenticateOAuthClient` (`oauth_token.go:70-74`) recusa sempre com `invalid_client`, por desenho (nunca "sem segredo = sem autenticação exigida").
- **Bloqueio estrutural, não só de configuração**: `oauthPasswordGrant` (`oauth_token.go:154-166`) rejeita explicitamente contas `aluno`/`encarregado`/`candidato` ("ainda não migaram para /oauth/token — use /api/auth/login"), só aceitando `funcionario`/`superadmin`. Mas `AdminAuthController::login()` / `AdminSession::login()`/`store()` é o **mesmo** formulário/sessão partilhado por **todos** os tipos de conta do site (ERP, escola, portal do aluno, do encarregado, do professor, do candidato — `AdminSession::store()` trata `body['aluno']`/`body['encarregado']`/`body['candidato']` explicitamente). Trocar `authenticate()` para `/oauth/token` sem mais nada quebraria login de aluno/encarregado/candidato em produção.
- **Ainda por decidir/implementar** (não avançado nesta sessão — a decidir com o utilizador antes de tocar no login partilhado):
  - Gerar e definir `client_secret_hash` de `web-erp` (local dev vs. produção — produção exige gestão de segredo fora do controlo de versões, mesmo padrão de `JWT_SECRET_KEY`/`GATEWAY_SHARED_SECRET`).
  - Estratégia de login: tentar `/oauth/token` primeiro e cair para `/api/auth/login` quando o Go rejeitar (contas de portal), populando `user`/`modulos`/`tipo`/`escopo` via `GET /api/auth/me` + `GET /api/auth/me/acesso` no caminho OAuth2 (mesmo padrão do Android, secção 8.4) — ou manter tudo em `/api/auth/login` por agora e só preparar o cliente OAuth2 sem o ligar.
  - `PhpSessionTokenProvider`/`AdminSession::refresh()` precisam de saber qual mecanismo a sessão usou (OAuth2 vs. legado) para chamar o endpoint de refresh certo.
