-- Actualiza auth.criar_cargos_padrao() para incluir cargos e permissões
-- granulares do módulo POS (Operador de Caixa, Caixa Sénior, Supervisor POS,
-- Administrador POS) e converte o cargo legado "Caixa" para as novas ações.
-- Baseada no conteúdo de 20260727000002_cargos_padrao_restore.up.sql.
...
BEGIN
    ...
    INSERT INTO auth.permissoes_cargo (cargo_id, modulo, acao) VALUES
        (v_id, 'pos', 'operar_pos'),
        (v_id, 'pos', 'abrir_sessao'),
        (v_id, 'pos', 'fechar_sessao'),
        (v_id, 'pos', 'registar_venda'),
        (v_id, 'pos', 'processar_devolucao'),
        (v_id, 'pos', 'movimentar_caixa'),
        (v_id, 'pos', 'aplicar_desconto'),
        (v_id, 'tesouraria','ver')
    ON CONFLICT DO NOTHING;
    ...
    -- ═══════════════════════════════════════════════════════════════════════
    -- 4.7  POS (PONTO DE VENDA)
    -- ═══════════════════════════════════════════════════════════════════════

    -- Operador de Caixa
    INSERT INTO auth.cargos (tenant_id, nome, descricao)
    VALUES (p_tenant_id, 'Operador de Caixa', 'Opera o terminal de venda, abre e fecha a própria sessão.')
    ON CONFLICT (tenant_id, nome) DO UPDATE SET descricao = EXCLUDED.descricao
    RETURNING id INTO v_id;
    ...
    -- Caixa Sénior
    INSERT INTO auth.cargos (tenant_id, nome, descricao)
    VALUES (p_tenant_id, 'Caixa Sénior', 'Opera o terminal e pode cancelar vendas.')
    ON CONFLICT (tenant_id, nome) DO UPDATE SET descricao = EXCLUDED.descricao
    RETURNING id INTO v_id;
    ...
    -- Supervisor POS
    INSERT INTO auth.cargos (tenant_id, nome, descricao)
    VALUES (p_tenant_id, 'Supervisor POS', 'Supervisiona caixas, gera relatórios e pode fechar sessões de outros.')
    ON CONFLICT (tenant_id, nome) DO UPDATE SET descricao = EXCLUDED.descricao
    RETURNING id INTO v_id;
    ...
    INSERT INTO auth.permissoes_cargo (cargo_id, modulo, acao) VALUES
        (v_id, 'pos', 'ver'),
        (v_id, 'pos', 'relatorios'),
        (v_id, 'pos', 'supervisionar'),
        (v_id, 'pos', 'fechar_outra_sessao')
    ON CONFLICT DO NOTHING;

    -- Administrador POS
    INSERT INTO auth.cargos (tenant_id, nome, descricao)
    VALUES (p_tenant_id, 'Administrador POS', 'Configura terminais, catálogo e descontos do POS.')
    ON CONFLICT (tenant_id, nome) DO UPDATE SET descricao = EXCLUDED.descricao
    RETURNING id INTO v_id;
