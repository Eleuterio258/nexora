...
app.post("/send-email/bulk", requireApiKey, bulkLimiter, async (req, res) => {
  ...
  if (!Array.isArray(emails) || emails.length === 0) {
    return res
      .status(400)
      .json({ error: "Campo 'emails' deve ser um array não vazio" });
  }

  if (emails.length > MAX_BULK_ITEMS) {
    return res
      .status(400)
      .json({ error: `Máximo de ${MAX_BULK_ITEMS} emails por pedido` });
  }
  ...
  for (let i = 0; i < emails.length; i++) {
    ...
    if (!emailRegex.test(to)) {
      return res
        .status(400)
        .json({ error: `Item ${i}: endereço 'to' inválido` });
    }
  }
